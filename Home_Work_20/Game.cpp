...
namespace UI
{
	...
	//DONE
	void printMunchkinWonSpecialItem()
	{
		std::cout << "CONGRATS: MunchkinG WON by Special Item! ()=(=======>\n";
	}
	//DONE
	...
	void printFriendWon() 
	{
		std::cout << "CONGRATS: Now you are friends :) \n";
	}
	//DONE
	...
	void printMunchkinLost(Monster* monster)
	{
		...
		//DONE
		std::cout << "-------- Runaway consequences--------" << std::endl;
		std::cout << "--------" << monster->getRunawayPolicy()->getFullInfo() << "--------" << std::endl << std::endl << std::endl;
		//DONE
	}
	...
	void printMonsterInfo(Monster* monster)
	{
		...
		//DONE
		std::cout << "-------- In case of runaway --------" << std::endl;
		std::cout<<"--------" << monster->getRunawayPolicy()->getFullInfo() << "--------" << std::endl << std::endl << std::endl;

		std::cout << "-------- In case of win --------" << std::endl;
		std::cout << "--------" << monster->getVictoryPolicy()->getFullInfo() << "--------" << std::endl << std::endl << std::endl;
		//DONE
	}
...
void Game::run()
{
	...
	constexpr int WinningLevel = 100;
	...
	while (m_munchkin.getLevel() < WinningLevel)
	{
		...
		while (!fight.getFinish())
		{
			...
			//DONE
			if (fight.getFriendStatus())
			{
				fight.victoryFlow(this->getItemsBonusDeck(), this->getModifiersBonusDeck());
				UI::printFriendWon();
				UI::pressAnyKeyToContinue();
			}

			
			else if (fight.getInstantVictory())
			{
				fight.victoryFlow(this->getItemsBonusDeck(), this->getModifiersBonusDeck());
				UI::printMunchkinWonSpecialItem();
				UI::pressAnyKeyToContinue();
			}
			//DONE

			else if (munchkinMonsterDifference >= 0)
			{
				fight.victoryFlow(this->getItemsBonusDeck(), this->getModifiersBonusDeck());
				UI::printMunchkinWon();
				UI::pressAnyKeyToContinue();
